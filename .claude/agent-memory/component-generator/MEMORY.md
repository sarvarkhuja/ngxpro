...
- Importing `@ngxpro/cdk` in secondary entries of `@ngxpro/components` works fine
- Importing `@ngxpro/core` in secondary entries of `@ngxpro/components` causes a TypeScript internal crash during partial compilation (ng-packagr TS bug: "Cannot destructure property 'pos' of 'file.referencedFiles[index]'")
- Root cause: `@ngxpro/core` imports `@ngxpro/cdk` internally; the cross-dep chain triggers a TS diagnostic format crash
- Solution: Tokens needed by `@ngxpro/components` secondary entries must live in `@ngxpro/cdk` or be defined locally in the entry point itself
...
Projects should use relative imports to import from other files within the same project. Use "./path/to/file" instead of import from "@ngxpro/cdk"eslint@nx/enforce-module-boundaries
