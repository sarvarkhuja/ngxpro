...
Use this when doing **agent-style optimization or rewrite** of ngxpro. Adopt the role that matches the task; follow the rules in `.cursor/rules/` and the specs in `.claude/`.
...
| CDK utilities (focus, DOM, observables) | **CDK Agent** | `taiga-family/cdk/` (patterns), `.cursor/rules/ngxpro-core-library.mdc` |
| Core services, theme, tokens, base components | **Core Foundation Agent** | `taiga-family/core/` (patterns), `.cursor/rules/ngxpro-core-library.mdc` |
...
| Composed blocks (charts, tables, forms, metrics) | **Block Generator Agent** | `.cursor/rules/ngxpro-block-standards.mdc`, `.claude/BLOCK_CATALOG.md` |
| Fintech blocks or templates | **Fintech Domain Agent** | `.cursor/rules/ngxpro-fintech.mdc` |
| README, TypeDoc, Storybook, showcase | **Documentation Agent** | `.cursor/rules/ngxpro-documentation.mdc` |
| Unit/E2E/a11y tests, coverage | **Testing Agent** | `.cursor/rules/ngxpro-testing.mdc` |
| CLI commands, generators, schematics | **CLI Generator Agent** | `.claude/NGXPRO_AGENTS.md` |
| Planning, consistency, quality | **Master Agent** | `.claude/NGXPRO_AGENTS.md` |

---

## Workflow (for multi-step work)

1. **Foundation**: Architecture → CDK → Core (Nx + Tailwind + @ngxpro/cdk + @ngxpro/core)
...
Full agent specs: `.claude/NGXPRO_AGENTS.md`, `.claude/AGENT_IMPLEMENTATION.md`, `.claude/BLOCK_CATALOG.md`, `.claude/DEVELOPER_GUIDE.md`.
